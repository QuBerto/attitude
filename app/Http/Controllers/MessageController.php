...
class MessageController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach($secrets as $secretKey) {
            ...
            $requestMessage = json_decode($request);
            if (!$requestMessage){
            ...
            $message->account_type = $requestMessage->accountType;
            $message->message_type = $requestMessage->systemMessageType;
            $message->clan_id = $clan->id;

            if (isset($requestMessage->clanTitle)) {
                $message->clanTitle = $requestMessage->clanTitle;
            }

            $message->created_at = $requestMessage->timestamp;
            Log::info('Request', [
                'message' => $message,
