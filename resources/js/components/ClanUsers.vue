    ...
    setup(props) {
        ...
        const addUser = async () => {
            try {
                ...
                const response = await axios.post(`${apiUrl}/api/clan/${props.clanId}/add-user`, newUser.value);
                users.value.push(response.data.user);
                newUser.value.discord_id = '';
                console.log(response.data);
                if (response.data.success) {
                    window.location.href = `${apiUrl}/clan/${props.clanId}`;
                }
            } catch (error) {
                console.error(error);
            }
        ...
        const removeUser = async (userId) => {
            if (confirm('Are you sure you want to remove this user?')) {
                try {
                    const apiUrl = import.meta.env.VITE_APP_URL;
                    const response = await axios.delete(`${apiUrl}/api/clan/${props.clanId}/remove-user/${userId}`);
